...
pub struct MultiThreadDataLoader<I, O, L: Label> {
    ...
    workers: OnceLock<WorkerPool<O, L>>,
}
...
type WorkerCommand<O, L: Label> = SyncSender<Message<O, L>>;

struct WorkerPool<O, L: Label> {
    /// One command channel per worker; sending a per-epoch sender starts a pass.
    senders: Vec<mpsc::Sender<WorkerCommand<O, L>>>,
    handles: Vec<thread::JoinHandle<()>>,
    item_counts: Vec<usize>,
}

impl<O, L: Label> Drop for WorkerPool<O, L> {
    fn drop(&mut self) {
    ...
    L: Label,
{
    ...
    fn workers(&self) -> &WorkerPool<O, L> {
        self.workers.get_or_init(|| {
            ...
            for (index, dataloader) in dataloaders.iter().enumerate() {
                ...
                let (command_sender, command_receiver) = mpsc::channel::<WorkerCommand<O, L>>();
